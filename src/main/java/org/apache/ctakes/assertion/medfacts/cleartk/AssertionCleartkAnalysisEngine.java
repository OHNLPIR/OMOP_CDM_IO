...
import org.apache.ctakes.assertion.medfacts.cleartk.perf.AnnotationCache;
...
/**
 * Adapted from Apache cTAKES version with performance enhancements (search: mod::perf)
 */
...
//          new CleartkExtractor.Bag(new CleartkExtractor.Following(10))
    ...
    public void process(JCas jCas) throws AnalysisEngineProcessException {
        ...
        String domainId = "";
        // mod::perf initialize annotation cache for this document on all annotations
...
//      instance.addAll(tokenFeatureExtractor.extract(jCas, entityMention));
      ...
      */
      ...
      for (ContextExtractor<BaseToken> extractor : this.tokenContextFeatureExtractors) {
        ...
        */
            // mod::perf use annotation cache instead of expensive covering
            LinkedList<Sentence> sents = new LinkedList<>(index.getCovering(entityOrEventMention.getBegin(), entityOrEventMention.getEnd(), Sentence.class));
...
//      List<Sentence> sents = new ArrayList<Sentence>(coveringSents.get(entityOrEventMention));
                // mod::perf Annotation Cache instead of selectCovered
                List<AssertionCuePhraseAnnotation> cues = new LinkedList<>(index.getCovered(coveringSent.getBegin(), coveringSent.getEnd(), AssertionCuePhraseAnnotation.class));
...
//      List<Feature> lcFeats = new ArrayList<Feature>();
            ...
            for (Feature feat : feats) {
                if (feat instanceof TreeFeature || (feat.getName() != null && (feat.getName().startsWith("TreeFrag") || feat.getName().startsWith("WORD") || feat.getName().startsWith("NEG"))))
                    continue;
                if (feat.getName() != null && (feat.getName().contains("_TreeFrag") || feat.getName().contains("_WORD") || feat.getName().contains("_NEG")))
                    continue;
        ...
        // mod::perf clean up document from annotation cache
