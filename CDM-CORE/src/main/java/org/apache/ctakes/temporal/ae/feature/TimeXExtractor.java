...
/** Modified version of TimeXExtractor with performance fixes (search: mod::perf) */
...
//		}
        ...
        if (sentList != null && !sentList.isEmpty()){
            ...
            for(Sentence sent : sentList) {
                // mod::perf use index instead x3
                for (TimeMention time : index.getCovered(sent.getBegin(), sent.getEnd(), TimeMention.class)) {
                    timeDistMap.put(Math.abs(time.getBegin() - annotation.getBegin()), time);
                }
                for (TimeAnnotation time : index.getCovered(sent.getBegin(), sent.getEnd(), TimeAnnotation.class)) {
                    timeDistMap.put(Math.abs(time.getBegin() - annotation.getBegin()), time);
                }
                for (DateAnnotation time : index.getCovered(sent.getBegin(), sent.getEnd(), DateAnnotation.class)) {
                    timeDistMap.put(Math.abs(time.getBegin() - annotation.getBegin()), time);
                }
            ...
            for (Map.Entry<Integer, IdentifiedAnnotation> entry : timeDistMap.entrySet()) {
                ...
                // mod::perf index lookup
                for (TreebankNode treebankNode : index.getCovering(entry.getValue().getBegin(), entry.getValue().getEnd(), TreebankNode.class)) {
