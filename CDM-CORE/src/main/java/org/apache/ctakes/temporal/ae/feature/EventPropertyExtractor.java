...
/** Modified version of EventPropertyExtractor with performance fixes (mod::perf) */
...
//			features.add(new Feature("docName", fname));
        ...
        // mod::perf lookup from index instead and remove redundant collection fill
        for(Sentence coveringSent : index.getCovering(event.getBegin(), event.getEnd(), Sentence.class)){
            List<EventMention> events = new LinkedList<>(index.getCovered(coveringSent.getBegin(), coveringSent.getEnd(), EventMention.class));
